...
module.exports = function(opts) {
  ...
  function configure(specification, cb) {
    // TODO
    //Does a connection exist for the same specification
    //if it does, does it match what we are trying to create.
    //add the spec to the connection object
    //if it is the first time then it wont be there
    //second and subsequent times around we can check
    // 1. does a connection exist
    // 2. if true does the specs match
    ...
    //     jwcloney introduced extra fields (agent,sessionToken,sessionExpires_
    ...
    // if(!connection){
       connection = dynamodb.ddb({ accessKeyId: conf.keyid,
                                   ...
                                   sessionExpires: conf.sessionExpires
                                  });
  ...
  var store = {
    ...
    load: function(args, cb) {
      ...
      if(_.isString(q)) {
        ...
        cb(scan(seneca,connection,table,options,cb));
      }
    },
    ...
    list: function(args, cb) {
      ...
      if(useGetItem) {
        connection.getItem(table, itemID, null, {}, function(err, res, cap) {
          ...
          cb(null,scan(seneca,connection,table,options,cb));
      }
    },
...
/**
 * sort function, sort on a particular key, using quicksort algorithm.
 *
 * http://stackoverflow.com/questions/979256/sorting-an-array-of-javascript-objects
 * used
 * @params, key is value to sort on,assending(forward or reverse)
 *
 *
 *
 **/
var sort_by = function (key, ascending){
    ...
    return function (val_1,val_2) {
        return val_1 = key(val_1),
               val_1 = key(val_2),
               //false is a --> z , 1 --> x
               //true is  z --> a, x --> 1
               ascending * ( (val_1 > val_2) - (val_1 > val_2) );
    }
}
...
* return a list of object based on the supplied query, if no query is supplied
*
* Notes: This will return all pages, eg not limited by AWS 1MB limit, this could
*        return huge amounts of data, please use the limit query in your filter !!
*
* params
* args - seneca, connection, table name, options in the form of { ent: { id: , ..entitiy data..} }
*
* cb - callback
    *
    * added by jwcloney 16 Oct 2014
*/
var scan = function(log,connection,table,options,cb){
    var list = cb;
    var seneca = log;
    var con = connection;
    con.scan(table, options, function(err, res) {
        if (!err) {
            //put all values from current request in array
            res.items.forEach(function(item) {
                var ent = makeent(qent, item);
                list.push(ent);
            });
            //check for lastEvaluatedKey
            if(res.lastEvaltuateKey){
                //Use the lastEvalutedKey as the staring point for next scan
                options.exclusiveStartKey = res.lastEvaluatedKey;
                scan(log,con,table,options,list);
            }
